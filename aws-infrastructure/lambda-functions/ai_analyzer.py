#!/usr/bin/env python3
"""
Damien AI Analyzer Lambda Function
...
def rule_matches_analysis(rule: Dict, analysis: Dict) -> bool:
    ...
    try:
        rule_conditions = rule.get('rule_definition', {}).get('conditions', {})
        classification = analysis.get('classification', {})
        features = analysis.get('features', {})
        
        # Check category match
        required_category = rule_conditions.get('content_analysis', {}).get('ai_classification')
        if required_category and required_category != classification.get('primary_category'):
            return False
        
        # Check confidence threshold
        confidence_threshold = rule_conditions.get('content_analysis', {}).get('confidence_threshold', 0.5)
        if analysis.get('confidence', 0) < confidence_threshold:
            return False
        
        # Check sender domain patterns
        sender_patterns = rule_conditions.get('sender_patterns', {})
        if sender_patterns:
            domains = sender_patterns.get('domains', [])
            ...
            for domain_pattern in domains:
                if domain_pattern.startswith('*@'):
                    ...
                    if sender_domain == domain_pattern[2:]:
                        domain_match = True
                        break
                elif domain_pattern == sender_domain:
                    domain_match = True
                    break
        ...
        logger.warning(f"Error checking rule match: {e}")
        return False
...
def calculate_match_confidence(rule: Dict, analysis: Dict) -> float:
    ...
    if rule_conditions.get('sender_patterns'):
        specificity_bonus += 0.1
    if rule_conditions.get('subject_patterns'):
        specificity_bonus += 0.1
    if rule_conditions.get('content_analysis'):
